...
class Repository(object):
    ...
    def all(self):
        """ Return repositories accessible from a user."""
        url = self.bitbucket.url('GET_USER_REPOS', username=self.bitbucket.username)
        response = self.bitbucket.dispatch('GET', url, auth=self.bitbucket.auth)
        try:
            return (response[0], response[1]['repositories'])
        except TypeError:
    ...
    def create(self, repo_name, scm='git', private=True, username=None, **kwargs):
        ...
        return self.bitbucket.dispatch('POST', url, auth=self.bitbucket.auth, owner=username, name=repo_name, scm=scm, is_private=private, **kwargs)
